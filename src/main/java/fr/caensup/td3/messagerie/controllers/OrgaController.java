  ...
  public String indexAction() {
    ...
    vue.addMethod("updateOrgaSubmit",
        orgaService.ifFormIsValid(
            Http.put(orgaService.getURL(restURL, "this.orga.id"), (Object) "this.orga",
                vue.assignOriginalWithHttp("orga")
                    + orgaService.toast("success", "Organisation ${this.orga.name} modifiée.")
                    + vue.set("orga", "null"))));
    vue.addMethod("addOrUpdate",
        "if(this.orga.id){this.updateOrgaSubmit();}else{this.newOrgaSubmit();}");
    vue.addMethod("popup", ";" + Http.get(orgaService.getURL(restURL, "orga.id") + "+'/groups'",
        "this.groups=response.data;"), "orga");
